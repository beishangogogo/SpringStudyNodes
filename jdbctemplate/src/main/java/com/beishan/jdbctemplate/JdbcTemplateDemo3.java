...
//        3、执行操作
//        保存
//        jt.update("insert into account(name,money)value(?,?)","eee",3000);
//        更新
//        jt.update("update account set name=?,money=? where id=?","test",1111,2);
//        删除
//        jt.update("delete from account where name=?", "ddd");
//        查询所有
...
//        System.out.println(accounts.isEmpty() ? "没有内容" : accounts.get(1));
...
class AccountRowMapper implements RowMapper<Account> {
    ...
    /**
     * 把结果集合中的数据封装到Account中，然后由spring把每个Account加到集合中
     * @param resultSet
     * @param i
     * @return
     * @throws SQLException
